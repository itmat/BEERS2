class Sample:
    ...
    def serialize(self):
        ...
        adapter_sequences_tuple = ",".join([str(adapter_sequence) for adapter_sequence in self.adapter_sequences])
        fastq_file_paths_tuple = ",".join([str(input_file_path) for input_file_path in self.fastq_file_paths])
        return f"{self.sample_id}\t{self.sample_name}\t{self.gender}\t{self.pooled}\t{fastq_file_paths_tuple}\t{self.bam_file_path}\t{adapter_sequences_tuple}"
    ...
    def deserialize(data):
        ...
        sample_id, sample_name, gender, pooled, fastq_file_paths_str, bam_file, adapter_sequences_str = \
            data.rstrip().split('\t')
